...
#undef LOG_TAG
#define LOG_TAG "SharedBufferClientR"
...
int main(int argc, char **argv) {
	...
	size_t size = mb->size();
	size_t offsize = mb->offset();
	ALOGD("%s() LINE:%d Get IMemory->size() -> %d - %d\n", __func__, __LINE__, size, offsize);
	ALOGD("%s() LINE:%d read data: %s\n", __func__, __LINE__, buffer);
